...
export const TarggetMet = ({ met }) => {
    ...
    const hanldeSelect = (overrideAmount = null) => {
        onSelect();
        setscreenMet(false);
        console.log('Selected Product Object:', JSON.stringify(met, null, 2));

        const amountToAdd = overrideAmount || peso;

        if (!amountToAdd) {
            // Check if item is sold by piece (and no specific weight option selected)
            const { piezas, piece } = met;
            const isPiece = (piezas === true || piezas === 'true' || piece === true || piece === 'true');
            console.log('Is Piece Result:', isPiece);

            if (isPiece) {
                // Default to 1 unit/kg for piece items
                const defaultAmount = 1;

                setProducts([...products, {
                    name: nombre,
                    cost: precio_de_venta * defaultAmount,
                    amount: defaultAmount,
                    id,
                    producto_id_cesta: new Date().getTime()
                }]);
                const newTotal = total + (precio_de_venta * defaultAmount);
                setTotal(newTotal);
